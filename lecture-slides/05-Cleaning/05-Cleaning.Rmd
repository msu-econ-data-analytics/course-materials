---
title: "Lecture 5: Data Wrangling in the Tidyverse"
...
1. [Variables & Values](#variables-values)
...
This section is drawn partly from [*R for Data Science*](https://r4ds.had.co.nz/index.html), chapter 13.
...
Factors: Does it save memory to store a variable as a factor instead of a string?
...
Note: The rest of the examples in this section are illustrative and not actually executable.

</br>

Convert variable to numeric:
```{r, eval=F}
data |> mutate(height = as.numeric(height))
```

Convert variable to integer:
```{r, eval=F}
data |> mutate(id_num = as.integer(id_num))
```

Convert all numeric variables to factors:
```{r, eval=F}
data |> mutate(across(is.numeric, as.factor))
```
...
students |>
  mutate(stu_label = recode(student, \`0` = NA_character_,
                                ...
                                .default = "student"))
```
`.default` matches all values not otherwise matched.
Otherwise, unmatched values are left unchanged.

You have to enclose in backticks the numeric values you're trying to match.

---

# Converting values

Set a certain value to missing: `dplyr::na_if`
```{r, eval=FALSE}
students |>
...
Set missing values to a certain value: `tidyr::replace_na`
```{r, eval=FALSE}
students |>
  mutate(student = replace_na(student, 0))
```
...
Use `dplyr::bind_rows` to append two datasets with the same variables.
```{r, eval=FALSE}
bind_rows(x, y)
```
...
This section is drawn partly from [*R for Data Science*](https://r4ds.had.co.nz/index.html), chapter 15.
...
Using `forcats`, part of the tidyverse.
* "cats" = categories
...
You can also skip the step of defining the levels. If you omit the second argument, R will just take the levels from your variable. The catch is that they'll be in alphabetical order.
...
If you'd prefer that the order of the levels match the order of the first appearance in the data, you can set levels to `unique(x)`, or after the fact, with `fct_inorder()`:
...
This section is drawn from the JHU Data Science Lab's [Tidyverse Skills for Data Science](https://jhudatascience.org/tidyversecourse/), section 3.7, and [*R for Data Science*](https://r4ds.had.co.nz/index.html), chapter 14.
...
`stringr` is a core tidyverse package that helps a lot with strings.
...
Count matches of a specified pattern: `str_count()`
```{r}
names = c("Keisha", "Mohammed", "Jane", "Mathieu")


# Identify strings that CONTAIN a lowercase "h"
str_count(names, "h")

# Identify strings that START with a capital "M"
str_count(names, "^M")
```
...
# Identify strings that END with an uppercase "a"
str_count(names, "A$")

# Identify strings that END with a lowercase "a"
str_count(names, "a$")
...
Extract the portion of strings that match a specified pattern: `str_extract()`
```{r}
str_extract(names, "^M")
```
</br>
...
.pull-right[
```{r}
# Match anything NOT a lowercase vowel
str_view_all(names, "[^aeiou]")
```
]
...
.pull-left[
```{r}
# Match when n shows up 1+ times
str_view_all(addresses, "n+")
```
]
.pull-right[
```{r}
# Match when n shows up 2 times
...
.pull-left[
```{r}
# Use a vertical bar to alternate
str_view(g, "grey|gray")
```
]
.pull-right[
```{r}
# And parentheses to clarify precedence
str_view(g, "gr(e|a)y")
```
]
...
# Match "labor" if it contains either 0 or 1 "u"
str_view(l, "labo(u?)r")
...
```{r, eval=F}
customers |> 
  mutate(credit_card_spaces = str_remove_all(credit_card, " "))
```

Finding invalid credit card numbers:
```{r, eval=F}
customers |> 
...
Why is a credit card number stored as a string/character?
...
Often we want to categorize values into groups. To do that, we need to make sure values that should be the same, actually are.
...
```{r, eval=F}
animals |> mutate(type_upper = str_to_upper(type))

```

Fix whitespace inconsistency:
```{r, eval=F}
animals |> mutate(type_trimmed = str_trim(type))
