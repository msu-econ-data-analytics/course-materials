...
1. [Distributed computing with Spark](#spark)
...
```{r con}
connection = dbConnect(RSQLite::SQLite(), path = ":memory:")
```
...
```{r flights_db}
flights_db = tbl(connection, "flights")
class(flights_db)
flights_db
```
...
```{sql sql_direct_rmd, connection=connection}
SELECT *
FROM flights
WHERE dep_delay > 240
LIMIT 5
...
When you're done using a database, **disconnect** from the connection:
...
- Cheap: Analyze 1 TB for free each month, after that it's half a cent per GB.
