...
For prediction, we want "nice"-performing estimates $\hat y$ instead of $\hat\beta$.
...
We know how to define model performance, and we've discussed the need for separate training and testing data.

Now, how do we use these concepts to actually estimate MSE (or whatever your loss function is)?

Multiple answers, but the most important is .hi[resampling methods].

--

Resampling methods can be used for 2 important modeling tasks:
...
So far, we've treated each observation as separate/independent from each other observation. But exceptions are everywhere:
- Individuals in a household (cross-sectional data)
- Days of stock returns (time series data)
- Panel data (panel data)
- Pixels of land use categories (spatial data)

For cases like these, we need to modify the cross-validation procedures to take dependence into account.
- Time series data: Train on earlier data, validate on later data.
- Other types of data: Keep related groups of observations together when randomly dividing the data.

We'll see methods for doing this next week.
