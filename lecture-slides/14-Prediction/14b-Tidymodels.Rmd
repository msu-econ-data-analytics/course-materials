---
title: "Lecture 14b: Learning with tidymodels"
...
To fit the model to our data we could run:
```{r}
est_reg = model_lm %>%
  fit(Rating ~ ., data = credit_clean)
```

This will give you the exact same results as:
```{r, eval=F}
lm(Rating ~ ., data=credit_clean)
```
